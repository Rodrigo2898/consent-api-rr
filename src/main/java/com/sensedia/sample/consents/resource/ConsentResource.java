...
public class ConsentResource implements IConsentResource {
	...
	@PostMapping("/with-external-info")
	@Operation(summary = "Criar consentimento com informações externas")
	public Mono<ResponseEntity<ConsentResponse>> createWithExternalInfo(
			@Valid @RequestBody CreateConsent createConsent) {
		return consentService.createConsentWithAdditionalInfo(createConsent)
